...
public class GraphExamples {
 ...
 public static String[] printMatrix(String [][] edges, boolean directed)
  {
       //list of the vertecies
        ArrayList<String> Vertex = new ArrayList<>();
        Vertex.add(edges[0][0]);
        for (int i = 0; i < edges.length; i++) 
        {
            String temp = edges[i][0];
            int count = 0;
            for (int k = 0; k < Vertex.size(); k++) 
            {
                if(!(Vertex.get(k).equals(temp)))
                {
                    count++;
                }
                if(Vertex.get(k).equals(temp))
        ...
        String[] arr2 = new String[Vertex.size()];
        for (int i = 0; i < arr2.length; i++) 
        {
            arr2[i] = Vertex.get(i);
        }
        CompareName comp = new CompareName();
        arr2 = Sort.mergeSort(arr2, comp);
        for (int i = 0; i < arr2.length; i++) 
        {
          Vertex.set(i, arr2[i]);
        }
        ...
        //Final print
        int[][] arr2D = new int[Vertex.size()][Vertex.size() + 1];
        for (int i = 0; i < edges.length; i++) 
        {
            String vertex = edges[i][0];
            String oppositeVertex = edges[i][1];
            for (int j = 0; j < Vertex.size(); j++) 
            {
                if(Vertex.get(j).equals(vertex))
                {
                    for (int k = 0; k < arr2D.length; k++) 
                    {
                        if(oppositeVertex.equals(Vertex.get(k)) && directed == false)
                        {
                            arr2D[j][k] = 1;
                            arr2D[k][j] = 1;
                        }
                        if(oppositeVertex.equals(Vertex.get(k)))
                        {
                            arr2D[j][k] = 1;
                        }
                    }
        ...
        String[] arr = new String[Vertex.size() + 1];
        arr[0] = "|     |";
        for (int i = 0; i < Vertex.size(); i++) 
        {
          System.out.printf("  %4s |", Vertex.get(i));
        }
        System.out.printf("|      |");
        System.out.println("");
        for (int i = 1; i < Vertex.size() + 1; i++) 
        {
            System.out.printf("| %4s |", Vertex.get(i - 1));
            for (int j = 0; j < Vertex.size(); j++) 
            {
                System.out.printf("  %4d |", arr2D[i - 1][j]);
            }
            System.out.println("");
        }
        return arr;
  ...
  public static String[] printMatrixWithNumbers(String [][] edges, boolean directed)
  {
      //list of the vertecies
        ArrayList<String> Vertex = new ArrayList<>();
        Vertex.add(edges[0][0]);
        ...
        //Final print
        int[][] arr2D = new int[Vertex.size()][Vertex.size()];
        for (int i = 0; i < edges.length; i++) 
        {
            String vertex = edges[i][0];
            String oppositeVertex = edges[i][1];
            for (int j = 0; j < Vertex.size(); j++) 
            {
                if(Vertex.get(j).equals(vertex))
                {
                    for (int k = 0; k < arr2D.length; k++) 
                    {
                        if((oppositeVertex.equals(Vertex.get(k))) && (directed == false))
                        {
                            arr2D[j][k] = Integer.parseInt(edges[i][2]);
                            arr2D[k][j] = Integer.parseInt(edges[i][2]);
                        }
                        if(oppositeVertex.equals(Vertex.get(k)))
                        {
                            arr2D[j][k] = Integer.parseInt(edges[i][2]);
        ...
        for (int i = 1; i < Vertex.size() + 1; i++) 
        {
            ...
            for (int j = 0; j < Vertex.size(); j++) 
            {
                System.out.printf("  %4d |", arr2D[j][i - 1]);
            }
            System.out.println("");
        }
        return arr;
  ...
  public static Graph<String,Integer> figure14_3() {
    ...
    String[] temp = GraphExamples.printMatrix(edges, true);
  ...
  public static Graph<String,Integer> figure14_9() {
    ...
    String[] temp = GraphExamples.printMatrix(edges, false);
  ...
  public static Graph<String,Integer> figure14_14() {
    ...
    String[] temp = GraphExamples.printMatrixWithNumbers(edges, false);
    return graphFromEdgelist(edges, false);
  }
